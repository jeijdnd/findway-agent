...
app.whenReady().then(async () => {
  ...
  if (!backendReady) {
    ...
    const result = dialog.showMessageBoxSync({
      type: 'error',
      title: '启动失败',
      message: 'Python 后端启动失败',
      detail: '无法连接到 http://127.0.0.1:8765/api/health\n\n请检查：\n1. Python 是否已安装\n2. 虚拟环境是否已创建（运行 pip install -r requirements.txt）\n3. 端口 8765 是否被占用',
      buttons: ['确定', '重试'],
      defaultId: 0,
      cancelId: 1
    });
