...
当用户要筛选项目、切换项目阶段、打开/编辑/删除项目、查看项目列表时，必须调用 manage_projects 技能，不要回复没有相关工具。
...
class LLMEngine:
    ...
    def _map_tool_to_ui_action(
        ...
        if name == "manage_projects":
            ui_action = result.get("action")
            if not ui_action:
                return None, {}
            ui_data: Dict[str, Any] = {}
            for key in (
                "project_id", "project_name", "stage",
                "query", "filter_year", "filter_stage",
            ):
                if result.get(key) is not None:
                    ui_data[key] = result[key]
            return ui_action, ui_data
